...
function setup_Menu(canvas, menu, index, jndex) { // change to small cells only
        ...
        menu.obj = canvas.display.rectangle({
        	// dummy object to hold 9 small objects to create menu
            x: menu.active_Cell.parent.abs_x,
            y: menu.active_Cell.parent.abs_y
        });
                
        if (game.autoSweep == true) { //If auto sweep is on, get constraints       	
            var constraints = find_constraints(index, jndex);
        }                
        for (var i = 0; i < 3; i++){        	
             for (var j = 0; j < 3; j++){            	 
                 if (game.autoSweep == true) {                	 
                     var num = (j * 3) + i + 1;                     
                     if (constraints[num] == true) { //Only add number that are not constrained                    	 
...
function toggle_autoSweep() { //Function to toggle Auto Sweep
	if (game.autoSweep == false) {
		game.autoSweep = true;
	} else {
		game.autoSweep = false;
	}	
}
